...
from .triple_bus import TripleBusSystem, CollabTopic
...
# Consensus configuration (as designed with Comet)
CONSENSUS_CONFIG = {
    "winning_threshold": 0.60,      # 60% weighted votes to win
    "confidence_floor": 0.40,       # Reject proposals with confidence < 0.4
    "min_participants": 2,          # Require at least 2 agents
}
...
class CollaborationManager:
    ...
    async def _validate_consensus(
        ...
        """
        # Check: Enough votes?
        if vote_result["weight_percent"] < CONSENSUS_CONFIG["winning_threshold"]:
            return False, f"Insufficient votes ({vote_result['weight_percent']:.1%} < {CONSENSUS_CONFIG['winning_threshold']:.1%})"
        
        # Check: Confident enough? (Comet's key insight)
        if winner.get("confidence", 0.0) < CONSENSUS_CONFIG["confidence_floor"]:
            return False, f"Low confidence ({winner.get('confidence', 0.0):.2f} < {CONSENSUS_CONFIG['confidence_floor']})"
        
        # Check: Enough participants?
        if total_voters < CONSENSUS_CONFIG["min_participants"]:
            return False, f"Too few participants ({total_voters} < {CONSENSUS_CONFIG['min_participants']})"
